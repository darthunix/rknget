...
0. Весь код написан целиком на python3.7 с использованием библиотек из дистрибутивов. Временами возможна совместимость с python3.4.
...
0.3. Желательные требования: 4 vcpu 2.6 GHz, 6 GB RAM, 300 iops storage.
1. DB
...
3. Redis (опционально)
3.1. Разверните Redis, выделите ему от 128МБ памяти, настройте доступ.
3.2. LRU-cache, сброс на постоянный носитель - опционально.
4. Checkout
4.1. Настройте подключение к БД в файле rkn/dbconn.py
4.2. Настройте подключение к Redis в том же файле, либо проставьте в значениях None.
4.3. Можете проверить подключение запросом:
http://hostname:port/cgi-bin/rknget/rkn/webapi.py?module=api.monitoring&method=testConn
4.4. Можете настроить SSL, хотя проверка сертификата клиентом принудительно игнорируется.
...
- Обращения к API слишком ресурсоёмки, клиенты отваливаются по таймауту.
- Поставьте Redis для кэширования обращения к API, пока база не меняется.
